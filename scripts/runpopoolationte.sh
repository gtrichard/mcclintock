...
then
	...
	awk -F"\t" -v sample=$sample '{if($9~/-/) {printf "%s\t%d\t%d",$1,$2-1,$16; printf "\t"$13+$20"\t"$4;} else if($16~/-/) {printf "%s\t%d\t%d",$1,$10-1,$2; printf "\t"$13+$20"\t"$4;} else {printf "%s\t%d\t%d",$1,$10-1,$16; printf "\t"$13+$20"\t"$4;} if($7!="-") print "_reference_"$5"_"sample"_popoolationte_rp_\t0\t.\t"$3"\t"$11"\t"$18; else print "_non-reference_"$5"_"sample"_popoolationte_rp_\t0\t.\t"$3"\t"$11"\t"$18}' $outputfolder/PoPoolationTE/te-poly-filtered.txt > $outputfolder/PoPoolationTE/$sample"_popoolationte_presort_raw.txt"
